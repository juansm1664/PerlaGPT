...
@Component({
  ...
  template: `
    <a
      ...
      class="flex justify-center items-center hover:bg-gray-800 rounded-md p-2 trasition-colors"
    >
    <i class="{{ icon }} text-2xl mr-4 text-indigo-400"></i>
    <div class="flex flex-col flex-grow">
        <span class="text-white text-lg font-semibold">{{title}}</span>
        <span class="text-gray-400 text-lg font-semibold">{{ description }}</span>
    </div>
